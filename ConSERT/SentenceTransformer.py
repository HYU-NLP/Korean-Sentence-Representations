...
class SentenceTransformer(nn.Sequential):
    ...
    def fit(self,
        ...
        if steps_per_epoch is None or steps_per_epoch == 0:
            steps_per_epoch = min([len(dataloader) for dataloader in dataloaders]) #why len(dataloader)=93  ,, batch 덩어리크기들
        ...
        for loss_model in loss_models:
            ...
            optimizer_grouped_parameters = [
                {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay)], 'weight_decay': weight_decay},#group 0 
                {'params': [p for n, p in param_optimizer if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}  #group 1
            ] #n = name of parameters,  p = parameters
        
            optimizer = optimizer_class(optimizer_grouped_parameters, **optimizer_params) #optimizer 문법?
        ...
        num_train_objectives = len(train_objectives)  #unsup일때는 그냥 contrasitive loss만 쓰니까 1?
        ...
        for epoch in trange(epochs, desc="Epoch"):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, position=0, leave=True):
                ...
                ############# ADDED for early_stop #############
                    ...
                    for loss_model in loss_models:  # 내부적으로 eval 과정에서 model.encode가 호출되면서 model.eval이 활성화되기 때문.
                        loss_model.zero_grad()
                        loss_model.train()
