...
# 상속할 기반 클래스 : sentence_transformers.models.Transformer
class Transformer(models.Transformer): 
    ...
    def __init__(self, model_name_or_path: str, max_seq_length: int = 128,
        ...
        # super(파생 클래스, self)로 이게 어떤 클래스인지 명시 가능(기능은 super()와 동일)
        super(Transformer, self).__init__(model_name_or_path, max_seq_length, model_args, cache_dir, tokenizer_args, do_lower_case)
    # 1. 맨 위 __init__()에 있는 객체들 정의
    # 2. 그 중 super().__init__()에 있는 객체들을 기반 클래스에서 가져와 그대로 사용
    # 또한 self도 기반 클래스에서 가져왔기 때문에 또 정의할 필요가 없음
    # 그래서 self.config_keys, self.max_seq_length, self.auto_model, self.tokenizer를 그대로 사용가능
