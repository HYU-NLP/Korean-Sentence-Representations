...
# NTXentLoss를 제외한 나머지는 ablation study를 제외하고 쓸 일 없음

class NTXentLossOriginal(torch.nn.Module): # original NT-Xent loss
...
class NTXentLossOpt1(NTXentLossOriginal): # sentence embedding 간 영향, hidden representation 간 영향을 제거한 NT-Xent loss
...
class NTXentLoss(torch.nn.Module): # customize 끝낸 NT-Xent loss
    ...
    def forward(self, cls, cont):
        ...
        for i in range(num_layers):
            ...
            # add (batch_size, 1)
            positives.append(torch.diag(similarity_matrix)) # 행렬의 대각성분만 가져오면 같은 idx의 embedding끼리 similarity score 확인 가능
            # (batch_size, batch_size - 1)
            neg_idx = (1 - torch.eye(batch_size)).bool() # 대각성분 외 나머지 성분들은 서로 다른 idx의 embedding끼리의 similarity score
...
class Loss(nn.Module):
    def __init__(self, model, args):
        ...
        self.original = copy.deepcopy(model) # SentenceTransformer(BertModel(bert-base-uncased), Pooling())
        self.original[0].eval() # same as BertModel(bert-base-uncased)
    ...
    def compute_diff(self): # 논문의 L_reg = ||BERT_F-BERT_T||^2 (L2 norm)
        diff = 0.0
        for n,m in self.model[0].named_parameters():
            diff += torch.norm(self.original_params[n] - m, p=2) ** 2
        return diff
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels):
        ...
        loss1 = self.loss(sent_emb, pooled) # NTXentLoss
        if self.args.lamb > 0 :
            loss2 = self.compute_diff() # BERT_T가 BERT_F와 너무 떨어지지 않도록 하는 regularization
