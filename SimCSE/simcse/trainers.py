...
class CLTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        if resume_from_checkpoint is not None and os.path.isfile(
                os.path.join(resume_from_checkpoint, trainer.TRAINER_STATE_NAME)
        ):
            self.state = TrainerState.load_from_json(os.path.join(resume_from_checkpoint, trainer.TRAINER_STATE_NAME))
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    # Gradient clipping
                    if args.max_grad_norm is not None and args.max_grad_norm > 0 and not self.deepspeed and self.args.disable_gradient_clipping is False:
